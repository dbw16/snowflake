...
export interface PointsToLevels {
  [points: number]: string;
}

export const milestones: Milestone[] = [1, 2, 3, 4, 5];
...
export const pointsToLevels: PointsToLevels = {
  '0': '1.1',
  '5': '1.2',
  '11': '1.3',
  '17': '2.1',
...
export const tracks: { [key: string]: Track } = {
  MOBILE: {
    displayName: 'Mobile',
    category: 'A',
...
export const categoryPointsFromMilestoneMap = (milestoneMap: MilestoneMap): CategoryPoint[] => {
  let pointsByCategory = new Map<Category, number>();
  trackIds.forEach((trackId: TrackId) => {
...
export const totalPointsFromMilestoneMap = (milestoneMap: MilestoneMap): number =>
  trackIds
    .map((trackId: TrackId) => milestoneToPoints(milestoneMap[trackId]))
    .reduce((sum, addend) => sum + addend, 0);

export const categoryColorScale = d3
  .scaleOrdinal<string, string>()
  .domain(Array.from(categoryIds.values()))
